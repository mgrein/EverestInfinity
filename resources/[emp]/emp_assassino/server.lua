...
function emP.checkPayment()
	...
	if user_id then
		if vRP.getInventoryWeight(user_id)+vRP.getItemWeight("orgao") <= vRP.getInventoryMaxWeight(user_id) then
			...
			TriggerClientEvent("Notify",source,"sucesso", "Você recebeu <b>1x Orgão</b>.")
			return true
		end
	end
end
		...
		for l,w in pairs(soldado) do
			...
			if player then
				async(function()
					...
					TriggerClientEvent('chatMessage',player,"190",{65,130,255},"Recebemos a denuncia de um ladrão de orgão, verifique o ocorrido.")
