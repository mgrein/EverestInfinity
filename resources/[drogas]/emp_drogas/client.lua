	...
	while true do
		...
		if distance <= 2500 then
			...
			repeat
				...
				if not IsPedDeadOrDying(npc) and not IsPedAPlayer(npc) and distancia <= 2.0 and not IsPedInAnyVehicle(ped) and not IsPedInAnyVehicle(npc) and not pedlist[npc] and not selectnpc and GetPedType(npc) ~= 28 then
					if IsControlJustPressed(0,38) then
						if emP.checkItens() then
							...
							if porcentagem >= 1 and porcentagem <= 85 then
								segundos = 10
							elseif porcentagem >= 86 and porcentagem <= 95 then
								segundos = -1
								selectnpc = nil
								emP.MarcarOcorrencia()
							elseif porcentagem >= 96 then
								segundos = -1
								selectnpc = nil
								emP.MarcarOcorrencia()
