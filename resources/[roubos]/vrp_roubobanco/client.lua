...
----------------------------
local segurando_c4 = false
local c4_net = nil 
local segurando_bag = false
local bag_net = nil
local segurando_cartao = false
local cartao_net = nil
local segurando_laptop = false
local laptop_net = nil
local segurando_serra = false
local serra_net = nil
local segurando_furadeira = false
local furadeira_net = nil
	...
	while true do
		for k, v in ipairs(Config.BankRobbery) do
			...
			if Distancia < 6.0 then
				...
				TextoMarker(x,y,z+1.0, "PRESS ~r~F~w~ TO START THE THEFT", Opacidade, 0.54, 0.54)
				...
				if contador == 0 then
					if (IsControlJustPressed(1,49)) then
                        if RBserver.copsonline() >= v.Cops then
                            local message = '[POLICIAL] ASSALTO AO BANCO! TODAS AS UNIDADES NO '..v.BankName..'!'
                            ...
                            TriggerEvent('vrp_roubobanco:helpTimed', 'Número insuficiente de policiais.', 200)
                            RouboIniciado = false
						end
					end
				else
					TextoMarker(x,y,z+0.8, "WAIT ~r~".. contador .. " ~w~SECONDS TO STEAL AGAIN", Opacidade, 0.54, 0.54)
				end
			end
        end
        Citizen.Wait(1)
                ...
                if allowed then
					...
					local police = RBserver.copsonline()
                        ...
                        if police >= v.Cops then
                            ...
                            --loadModel(bagHash1)
                            --loadModel(lap_hash)
                            cardentity = CreateObject(card_hash, coords+20, true, false)
                            lapentity = CreateObject(lap_hash, coords+20, true, false)
                            ...
                            local netid = ObjToNet(card_hash)
                            local netid_2 = ObjToNet(lap_hash)
                            local netid_3 = ObjToNet(bagHash1)                           
                            SetNetworkIdExistsOnAllMachines(netid, true)
                            NetworkSetNetworkIdDynamic(netid, true)
                            SetNetworkIdCanMigrate(netid, false)
                            SetNetworkIdExistsOnAllMachines(netid_2, true)
                            NetworkSetNetworkIdDynamic(netid_2, true)
                            SetNetworkIdCanMigrate(netid_2, false)
                            SetNetworkIdExistsOnAllMachines(netid_3, true)
                            NetworkSetNetworkIdDynamic(netid_3, true)
                            SetNetworkIdCanMigrate(netid_3, false)
                            AttachEntityToEntity(bagProp1, PlayerPedId(), bagIndex3, 0.3, -0.25, -0.3, 300.0, 200.0, 300.0, true, true, false, true, 1, true)
                            segurando_bolsa = true
                            ...
                            DetachEntity(NetToObj(cartao_net), 1, 1)
                            DeleteEntity(NetToObj(cartao_net))
                            cartao_net = nil
                            ...
                            Wait(500)
                            ClearPedSecondaryTask(GetPlayerPed(-1))
                            DetachEntity(NetToObj(laptop_net), 1, 1)
                            DeleteEntity(NetToObj(laptop_net))
                            laptop_net = netid_2
                            segurando_laptop = true
                            ...
                            TriggerEvent("Notify","negado","Não há policiais suficientes online!")
                        end
                    end
                end
                Wait(10)
    ...
    while true do
        ...
        for i = 1, #Config.BankRobbery do
            ...
            for j = 1, #v.Doors do
                ...
                local player = PlayerPedId()
                local coords = GetEntityCoords(player)
                if GetDistanceBetweenCoords(coords, d.Coords, true) <= 1.0 and d.Frozen and RouboIniciado then
                    if true then
                        while GetDistanceBetweenCoords(GetEntityCoords(player), d.Coords, true) <= 1.0 and d.Frozen and not d.Hacking do
                            local allowed = false
                            if j == 1 then
                                allowed = true
                            else
                            ...
                            if allowed then
                                ...
                                if IsControlPressed(0, 38) then
                                    ...
                                    if police >= v.Cops then
                                    if j == 1 or j == 2 then
                                        if RBserver.getitem("c4flare") then
                                        ClearPedTasks(player)
                                        ...
                                        --loadModel(bagHash4)
                                        --Wait(10)
                                        thermalentity = CreateObject(thermal_hash, d.WeldPosition.C-20, true, false)
                                        --local netid_test = ObjToNet(thermalentity)
                                        ...
                                        Wait(2500)
                                        ClearPedSecondaryTask(GetPlayerPed(-1))
                                        DetachEntity(NetToObj(c4_net), 1, 1)
                                        DeleteEntity(NetToObj(c4_net))
                                        c4_net = nil
                                        segurando_c4 = false
                                    ...
                                    if RBserver.getitem("serra") then
                                        ...
                                        TriggerEvent("Notify","negado","Você não possui uma serra!")
                                    end
                                    end
                                    else
										TriggerEvent("Notify","negado","Não há policiais suficientes online!")
                        ...
                        if GetDistanceBetweenCoords(coords, s.Coords, true) <= 1.0 and not Config.BankRobbery[i].Doors[#Config.BankRobbery[i].Doors].Frozen then
                            ...
                            if IsControlPressed(0, 38) then
                                ...
                                if RBserver.getitem("furadeira") then
                                    ...
                                    TriggerEvent("Notify","negado","Você não possui uma furadeira!")
                                end    
                                else
									TriggerEvent("Notify","negado","Não há policiais suficientes online!")
                                end
