...
function src.buyDealer(name)
	...
	if user_id then
		...
		if vehicle[1] then
			...
			if vRP.hasPermission(user_id,"conce.permissao") then
				if vRP.tryFullPayment(user_id,parseInt(vRP.vehiclePrice(name)*0.60)) then
					...
					TriggerClientEvent("Notify",source,"sucesso","Você comprou um <b>"..vRP.vehicleName(name).."</b> por <b>$"..vRP.format(parseInt(vRP.vehiclePrice(name)*0.60)).." dólares</b>.",10000)
					TriggerEvent("everest:postarDiscord", source, DISCORD_WEBHOOK, "LOGS CSS", "[COMPROU]:"  ..vRP.vehicleName(name).. " por "..vRP.format(parseInt(vRP.vehiclePrice(name)*0.60)).." dólares</b>.", 16711680)
			...
			elseif vRP.hasPermission(user_id,"diamante.permissao") then
				if vRP.tryFullPayment(user_id,parseInt(vRP.vehiclePrice(name)*0.85)) then
					vRP.execute("creative/set_estoque",{ vehicle = name, quantidade = parseInt(rows2[1].quantidade) - 1 })
					vRP.execute("creative/add_vehicle",{ user_id = parseInt(user_id), vehicle = name })
					vRP.execute("creative/set_ipva",{ user_id = parseInt(user_id), vehicle = name, ipva = parseInt(os.time()) })
					TriggerClientEvent("Notify",source,"sucesso","Você comprou um <b>"..vRP.vehicleName(name).."</b> por <b>$"..vRP.format(parseInt(vRP.vehiclePrice(name)*0.85)).."Dolares</b>.",10000)
					TriggerEvent("everest:postarDiscord", source, DISCORD_WEBHOOK, "LOGS CSS", "[COMPROU]:"  ..vRP.vehicleName(name).. " por "..vRP.format(parseInt(vRP.vehiclePrice(name)*0.85)).." dólares</b>.", 16711680)
					src.updateVehicles(name,vRP.vehicleType(name))
					if vRP.vehicleType(name) == "carros" then
						TriggerClientEvent('dealership:Update',source,'updateCarros')
					elseif vRP.vehicleType(name) == "motos" then
			...
			elseif vRP.hasPermission(user_id,"platina.permissao") then
				if vRP.tryFullPayment(user_id,parseInt(vRP.vehiclePrice(name)*0.90)) then
					vRP.execute("creative/set_estoque",{ vehicle = name, quantidade = parseInt(rows2[1].quantidade) - 1 })
					vRP.execute("creative/add_vehicle",{ user_id = parseInt(user_id), vehicle = name })
					vRP.execute("creative/set_ipva",{ user_id = parseInt(user_id), vehicle = name, ipva = parseInt(os.time()) })
					TriggerClientEvent("Notify",source,"sucesso","Você comprou um <b>"..vRP.vehicleName(name).."</b> por <b>$"..vRP.format(parseInt(vRP.vehiclePrice(name)*0.90)).."Dolares</b>.",10000)
					TriggerEvent("everest:postarDiscord", source, DISCORD_WEBHOOK, "LOGS CSS", "[COMPROU]:"  ..vRP.vehicleName(name).. " por "..vRP.format(parseInt(vRP.vehiclePrice(name)*0.90)).." dólares</b>.", 16711680)
					src.updateVehicles(name,vRP.vehicleType(name))
					if vRP.vehicleType(name) == "carros" then
						TriggerClientEvent('dealership:Update',source,'updateCarros')
					elseif vRP.vehicleType(name) == "motos" then
			...
			elseif vRP.hasPermission(user_id,"ouro.permissao") then
				if vRP.tryFullPayment(user_id,parseInt(vRP.vehiclePrice(name)*0.95)) then
					vRP.execute("creative/set_estoque",{ vehicle = name, quantidade = parseInt(rows2[1].quantidade) - 1 })
					vRP.execute("creative/add_vehicle",{ user_id = parseInt(user_id), vehicle = name })
					vRP.execute("creative/set_ipva",{ user_id = parseInt(user_id), vehicle = name, ipva = parseInt(os.time()) })
					TriggerClientEvent("Notify",source,"sucesso","Você comprou um <b>"..vRP.vehicleName(name).."</b> por <b>$"..vRP.format(parseInt(vRP.vehiclePrice(name)*0.95)).." dólares</b>.",10000)
					TriggerEvent("everest:postarDiscord", source, DISCORD_WEBHOOK, "LOGS CSS", "[COMPROU]:"  ..vRP.vehicleName(name).. " por "..vRP.format(parseInt(vRP.vehiclePrice(name)*0.95)).." dólares</b>.", 16711680)
					src.updateVehicles(name,vRP.vehicleType(name))
					if vRP.vehicleType(name) == "carros" then
						TriggerClientEvent('dealership:Update',source,'updateCarros')
					elseif vRP.vehicleType(name) == "motos" then
