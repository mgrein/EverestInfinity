...
function vRPN.Mochila()
    ...
    if user_id then
        ...
        return inventario, vRP.getInventoryWeight(user_id),
        ...
        if nuser_id and vRP.itemIndexList(itemName) and itemName ~= "identidade" then
            if parseInt(amount) > 0 then
                if vRP.getInventoryWeight(nuser_id) +
                    vRP.getItemWeight(itemName) * amount <=
                    ...
                    if vRP.tryGetInventoryItem(user_id, itemName, amount) then
                        ...
                        vRPclient._playAnim(source, true,
                                            ...
                                            false)
                        TriggerEvent("everest:postarDiscord", source, DISCORD_WEBHOOK, "ENVIO DE ITENS",
                                            "[ID]: " .. user_id ..
                                                "\nEnviou para [ID]: " ..
                                                nuser_id .. " \nQuantidade: " ..
                                                amount .. "x " .. itemName,
                        ...
                        TriggerClientEvent("Notify", source, "sucesso",
                                           "Enviou <b>" .. vRP.format(amount) ..
                                               "x " ..
                                               vRP.itemNameList(itemName) ..
                                               "</b>.", 8000)
                        TriggerClientEvent("Notify", nplayer, "sucesso",
                                           "Recebeu <b>" .. vRP.format(amount) ..
                                               "x " ..
                                               vRP.itemNameList(itemName) ..
                                               "</b>.", 8000)
                        TriggerClientEvent('Creative:Update', source,
                                           'updateMochila')
                        TriggerClientEvent('Creative:Update', nplayer,
                                           'updateMochila')
                        return true
                    end
                end
                ...
                for k, v in pairs(inv) do
                    if itemName == k then
                        if vRP.getInventoryWeight(nuser_id) +
                            vRP.getItemWeight(itemName) * parseInt(v.amount) <=
                            vRP.getInventoryMaxWeight(nuser_id) then
                            if vRP.tryGetInventoryItem(user_id, itemName,
                                                       parseInt(v.amount)) then
                                vRP.giveInventoryItem(nuser_id, itemName,
                                                      parseInt(v.amount))
                                vRPclient._playAnim(source, true, {
                                    {"mp_common", "givetake1_a"}
                                }, false)
...
function vRPN.dropItem(itemName, amount)
    local DISCORD_WEBHOOK = "https://discordapp.com/api/webhooks/676632687888302101/ttjH15cGUmU9S_V3tA_pIO9jFkXQcrcYeNg_rdnmkh7_Z2Pg11URw_Z1X9MYp23y0cIn"
    local source = source
    if itemName ~= "identidade" then
        local user_id = vRP.getUserId(source)
        local x, y, z = vRPclient.getPosition(source)
        if parseInt(amount) > 0 and
            vRP.tryGetInventoryItem(user_id, itemName, amount) then
            TriggerEvent("DropSystem:create", itemName, amount, x, y, z)
            vRPclient._playAnim(source, true, {{"pickup_object", "pickup_low"}},
                                false)
            TriggerEvent("everest:postarDiscord", source, DISCORD_WEBHOOK, "DROP DE ITENS", "[ID]: " .. user_id ..
                                  "\nDropou: " .. amount .. "x " .. itemName,
                              16711680)
            ...
            for k, v in pairs(inv) do
                if itemName == k then
                    if vRP.tryGetInventoryItem(user_id, itemName,
                                               parseInt(v.amount)) then
                        TriggerEvent("DropSystem:create", itemName,
                                     parseInt(v.amount), x, y, z)
                        vRPclient._playAnim(source, true,
                                            {{"pickup_object", "pickup_low"}},
                                            false)
                        TriggerEvent("everest:postarDiscord", source, DISCORD_WEBHOOK, "DROP DE ITENS",
                                          "[ID]: " .. user_id .. "\nDropou: " ..
                                              parseInt(v.amount) .. "x " ..
                                              itemName, 16711680)
                        TriggerClientEvent('Creative:Update', source,
...
Citizen.CreateThread(function()
    while true do
        ...
        for k, v in pairs(bandagem) do
            if v > 0 then
                bandagem[k] = v - 5
                if v == 0 then bandagem[k] = nil end
            end
        end
        for k, v in pairs(remedio) do
            if v > 0 then
                remedio[k] = v - 5
                if v == 0 then remedio[k] = nil end
            end
        end
        for k, v in pairs(colete) do
            if v > 0 then
                colete[k] = v - 5
                if v == 0 then colete[k] = nil end
            end
        end
    end
end)
    ...
    if user_id and ramount ~= nil and parseInt(ramount) >= 0 and
        ...
        elseif type == "recarregar" then
            ...
            if uweapons[weaponuse] then
                if vRP.tryGetInventoryItem(user_id, "wammo|" .. weaponuse,
                                           parseInt(ramount)) then
                    local weapons = {}
