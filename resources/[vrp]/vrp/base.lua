...
function vRP.getPrioridade()
	...
	filtrarVip("moderador", 95)
	filtrarVip("Diamante", 90)
	filtrarVip("Platina", 80)
	filtrarVip("Ouro", 60)
	filtrarVip("Prata", 40)
	filtrarVip("Bronze", 20)
		...
		if user_id then
			...
			if not vRP.isBanned(user_id) then
				...
				if vRP.isWhitelisted(user_id) then
					...
					deferrals.done("Envie para a equipe de administração o seu ID: "..user_id.."Discord:https://discord.gg/everestrp")
