...
function Queue:NotFull(firstJoin)
	...
	--local canJoin = self.PlayerCount + self:ConnectingSize() < maxPlayers
...
Citizen.CreateThread(function()
	local function playerConnect(name,setKickReason,deferrals)
		...
		if #Queue.Priority == 0 then
			Queue.Priority = vRP.getPrioridade()
		end
