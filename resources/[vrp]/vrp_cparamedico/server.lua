...
func = {}
Tunnel.bindInterface("vrp_cparamedico", func)

funcCliente = Tunnel.getInterface("vrp_cparamedico")


vRP._prepare("vRP/insert_paramedico",
             "INSERT IGNORE INTO paramedico (paramedico_id,user_id,dkey,dvalue,img,valor,datahora,id_pai) VALUES (@paramedico_id, @user_id, @dkey, @dvalue, @img, @valor, NOW(),@id_pai)")
vRP._prepare("vRP/select",
             "SELECT p.id, p.dvalue, p.img, p.valor, DATE_FORMAT(p.datahora, '%d/%m/%Y %H:%i') datahora, u.name, u.firstname FROM paramedico p INNER JOIN vrp_user_identities u ON u.user_id=p.paramedico_id WHERE p.user_id = @user_id AND p.dkey=@dkey ORDER BY 1 DESC LIMIT 0,20")
vRP._prepare("vRP/select_all",
             "SELECT p.id, p.dvalue, u2.foto, p.valor, DATE_FORMAT(p.datahora, '%d/%m/%Y %H:%i') datahora, u.name as nomeParamedico,    u.firstname as sobrenomeParamedico,    u2.name as nomeForagido,    u2.firstname as sobrenomeForagido FROM Paramedico p INNER JOIN    vrp_user_identities u ON u.user_id = p.paramedico_id        INNER JOIN    vrp_user_identities u2 ON u2.user_id = p.user_id WHERE p.dkey=@dkey ORDER BY 1 DESC LIMIT 0,20")

vRP._prepare("vRP/select_foragidos",
             "SELECT p.id, p.user_id, p.dvalue, u2.foto, p.valor, DATE_FORMAT(p.datahora, '%d/%m/%Y %H:%i') datahora, u.name as nomeParamedico,    u.firstname as sobrenomeParamedico,    u2.name as nomeForagido,    u2.firstname as sobrenomeForagido FROM    paramedico p        INNER JOIN    vrp_user_identities u ON u.user_id = p.paramedico_id        INNER JOIN    vrp_user_identities u2 ON u2.user_id = p.user_id WHERE    p.dkey = 'foragido' AND u2.foragido = 1 ORDER BY 1 DESC")
...
vRP._prepare("vRP/delete_foragido",
             "DELETE FROM paramedico WHERE dkey = 'foragido' AND user_id = @user_id")
...
function func.setRegistro(passaporte, dkey, dvalue, img, valor, id_pai)
    local source = source
    local paramedico_id = vRP.getUserId(source)
    ...
    if (dkey == "foragido" and not isForagido and valor == 1) or dkey ~=
        ...
        vRP.execute("vRP/insert_paramedico", {
...
function func.setMulta(id, valor)
    ...
    if valor > 0 then
        ...
        local value = vRP.getUData(parseInt(id), "vRP:ficha")
        local ficha = json.decode(value) or 0
        vRP.setUData(parseInt(id), "vRP:ficha",
                     json.encode(parseInt(ficha) + parseInt(valor)))
...
function func.Identidade(user_id)
    ...
    if user_id then
        ...
        if identity then
            return identity, vRP.format(parseInt(myficha)), groupv
        end
    end
end
