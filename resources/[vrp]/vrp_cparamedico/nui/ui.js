...
Everest = {
    ...
    setPassaporte: function (identity, multas, emprego) {
        ...
        dados = dados.replace("{CONSULTAS}", multas);
    ...
    setMulta: function () {
        Swal.mixin({
            ...
            {
                title: 'Etapa 1',
                text: 'Informe o motivo da consulta!',
                input: 'textarea',
            },
            {
                title: 'Etapa 2',
                text: 'Anexe uma imagem do cidadão!',
                input: 'file',
                inputAttributes: {
                    'accept': 'image/*',
            ...
            {
                title: 'Etapa 3',
                text: 'Informe o valor da consultar!',
                input: 'number',
            },
            ...
            setTimeout(function () {
                ...
                if (result.value[1] == null) {
                    Everest.Alert("success", "Prontuário Registrado com sucesso!", true);
    ...
    setOcorrencia: async function () {
        const { value: text } = await Swal.fire({
            input: 'textarea',
            inputPlaceholder: 'Escreva o prontuario aqui!',
            inputAttributes: {
                'aria-label': 'Escreva a prontuario aqui'
            },
            showCancelButton: true
        });
    ...
    setForagido: function (status) {
        ...
        var text = "Você desejar registrar o gesso!";
        if (status == 1) {
            text = "Remover gesso do cidadão?";
        }
        ...
        Swal.fire({
            ...
            if (result.value) {
                if (status == 0) {
                    ...
                    if (text != null) {
                        Everest.Alert("success", "Gesso colocado!", true);
                        setTimeout(function () {
                            Everest.sendData("ButtonClick", {
                                action: "setForagido",
                                passaporte: passaporte,
                    ...
                    status = 0
                    Everest.Alert("success", "Gesso removido!", true);
                    setTimeout(function () {
                        Everest.sendData("ButtonClick", {
                            action: "setForagido",
                            passaporte: passaporte,
    ...
    getFichaCriminal: function (prisoes, request) {
        var htmlprisoes = '<div class="row" id="multa_{ID}">' +
            ...
            '<br><strong>Paramedico:</strong> {PARAMEDICO}' +
        ...
        if (request) {
            ...
            if (prisoes.length == 0) {
                ...
                $(prisoes).each(function (index, data) {
                    ...
                    html = html.replace("{PARAMEDICO}", data.name + " " + data.firstname);
    ...
    setListaMultas: function (multas, request) {
        ...
        if (request) {
            ...
            if (multas.length == 0) {
                ...
                Everest.newModal("CONSULTAS", htmlAux);
            }
            // Swal.close();
        }
    },
            ...
            if (multas.length == 0) {
                ...
                $(multas).each(function (index, data) {
                    var html = '<div class="row" id="multa_{ID}">' +
                        ...
                        '<br><strong>Motivo:</strong> {MOTIVO}' +
                        '<br><strong>Paramedico Responsável:</strong> {PARAMEDICO}' +
                        '<br><strong>Data:</strong> {DATA}' +
                        '</div>' +
                    ...
                    html = html.replace("{MOTIVO}", data.dvalue);
                    html = html.replace("{PARAMEDICO}", data.nomeParamedico + " " + data.sobrenomeParamedico);
                    html = html.replace("{DATA}", data.datahora);
    ...
    getListaOcorrencias: function (multas, request, passaporte) {
        ...
        if (request) {
            ...
            if (multas.length == 0) {
                Everest.newModal("PRONTUÁRIOS", "<strong>Nada Encontrado</strong>");
            } else {
                var htmlAux = "";
                $(multas).each(function (index, data) {
                    var html = '<div class="row" id="multa_{ID}">' +
                ...
                Everest.newModal("Prontuário", htmlAux);
            }
    ...
    sendData: function (name, data, load) {
        ...
        setTimeout(function () {
            $.post("http://vrp_cparamedico/" + name, JSON.stringify(data));
        }, time);
    },
