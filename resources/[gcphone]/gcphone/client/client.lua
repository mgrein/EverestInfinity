...
vRPgc = Tunnel.getInterface("gcPhone")
...
function showFixePhoneHelper(coords)
    for number, data in pairs(FixePhone) do
        local dist = GetDistanceBetweenCoords(data.coords.x, data.coords.y,
                                              data.coords.z, coords.x, coords.y,
                                              coords.z, 1)
        if dist <= 2.0 then
            SetTextComponentFormat("STRING")
            AddTextComponentString("~g~" .. data.name .. ' ~o~' .. number ..
                                       '~n~~INPUT_PICKUP~~w~ Utilizar')
...
-- ==================================================================================== 
RegisterNUICallback('log', function(data, cb) cb() end)
...
AddEventHandler('vrp_addons_gcphone:call', function(data)
    ...
    if message ~= nil and message ~= "" then
        TriggerServerEvent('vrp_player:efeutarchamado', number, message)
    end
end)
