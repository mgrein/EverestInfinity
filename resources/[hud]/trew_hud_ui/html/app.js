...
function ajaxio(method, url, type = null, data = null) {
    return new Promise((resolve, reject) => {
        ...
        if (method == 'POST') { req.send(data); } else { req.send(); }
    });
}
...
window.onload = function() {
    ...
    var eventCallback = {
        ui: function(data) {
            ...
            if (config.showVoice == true) { document.querySelector('#voice').style.display = 'block'; } else if (config.showVoice == false) { document.querySelector('.info.time-and-place').style.right = '25px'; }

            if (config.showWeapons == true) { document.querySelector('#weapon').style.display = 'block'; }
        },
        element: function(data) {
            if (data.task == 'enable') { document.querySelector('#' + data.value).style.display = 'block'; } else if (data.task == 'disable') { document.querySelector('#' + data.value).style.display = 'none'; }
        },
        ...
        isTalking: function(data) {
            ...
            if (data.value) { voiceId.classList.add('speak'); } else { voiceId.classList.remove('speak'); }
        },
        ...
        createStatus: function(data) {
            ...
            if (document.getElementById(statusID)) {} else {
        ...
        setStatus: function(data) {
            ...
            if (data.isdead == true) {
                ...
                for (i = 0; i < data.status.length; i++) {
                    if ((data.status[i].name == 'hunger') || (data.status[i].name == 'thirst')) { var statusValue = Math.floor(data.status[i].value); } else { var statusValue = Math.floor(data.status[i].value); }

                    if (document.querySelector('#' + data.status[i].name + ' span')) {
                        if (data.status[i].name == "health") {
                            document.querySelector('#' + data.status[i].name + ' span').style.height = (statusValue / 3) + '%';
                        } else {
            ...
            if (data.status == true) {
                if (vehicleInfo.classList.contains('inactive')) {
                    ...
                    if (vehiclesCars.indexOf(data.type) > -1) {
                        ...
                        if (data.type == 13) { document.querySelector('#vehicle-others i').classList.add('fa-biking'); } else if (data.type == 14) { document.querySelector('#vehicle-others i').classList.add('fa-ship'); } else if (data.type == 15) { document.querySelector('#vehicle-others i').classList.add('fa-helicopter'); } else if (data.type == 16) { document.querySelector('#vehicle-others i').classList.add('fa-plane'); } else if (data.type == 21) { document.querySelector('#vehicle-others i').classList.add('fa-train'); }
                    }
                ...
                if (speedPercentage >= 0 && speedPercentage <= 25) { speedometerCircle.classList.add('zero'); } else if (speedPercentage > 25 && speedPercentage <= 50) { speedometerCircle.classList.add('twentyfive'); } else if (speedPercentage > 50 && speedPercentage <= 75) { speedometerCircle.classList.add('fifty'); } else if (speedPercentage > 75) { speedometerCircle.classList.add('seventyfive'); }
                ...
                if (data.siren == true) { document.querySelector('#vehicle-gear').classList.add('pulsing'); } else { document.querySelector('#vehicle-gear').classList.remove('pulsing'); }
                ...
                if (vehicleLights.classList.contains(data.lights) == false) {
                    ...
                    if (data.lights == 'high') { vehicleLights.querySelector('i img').src = 'img/vehicle-lights-high.png'; } else { vehicleLights.querySelector('i img').src = 'img/vehicle-lights.png'; }
                }
        ...
        sound: function(file = null, args = null) {
            ...
            if (soundFile == null) { sound.setAttribute('src', ''); } else {
                if (args == null) {} else {
                    for (var key in args) {
                        if (key != 'addMultiListener') {
                            if (key == 'volume') { sound.volume = args[key]; } else { sound.setAttribute(key, args[key]); }
                        }
                    }
                }
    ...
    document.querySelectorAll('.icon i').addMultiListener('webkitAnimationEnd mozAnimationEnd MSAnimationEnd oanimationend animationend', function() {
