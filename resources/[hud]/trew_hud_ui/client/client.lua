	...
	while true do
		...
		if Config.ui.showLocation == true then
			...
			locationMessage = string.format(
				Locales['br']['you_are_on_location'],
				locationMessage
			)
	...
	while true do
		...
		if IsPedInAnyVehicle(player, false) and vehicleIsOn then
			...
			if Config.vehicle.speedUnit == 'MPH' then
				...
				vehicleSpeed = math.ceil(vehicleSpeedSource * 1.6)
			end
			...
			if vehicleSpeed > Config.vehicle.maxSpeed then
				vehicleNailSpeed = math.ceil(  280 - math.ceil( math.ceil(Config.vehicle.maxSpeed * 105) / Config.vehicle.maxSpeed) )
			else
...
function trewDate()
	...
	local date_format = Locales['br']['date_format'][Config.date.format]
